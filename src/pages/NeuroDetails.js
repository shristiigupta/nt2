...
// ✅ Log visitor for this specific subpage
useEffect(() => {
if (videoName) {
logVisitor(`Neurotherapy - ${videoName}`);
}
}, [videoName]);
...
// Collect available sections dynamically
const sections = [];
if (details.video_hindi)
sections.push({ type: "Hindi Video", src: details.video_hindi });

if (details.video_english)
sections.push({ type: "English Video", src: details.video_english });

if (details.pdf) sections.push({ type: "PDF", src: details.pdf });

return ( <div className="disease-details"> <h1>{videoName}</h1>
    ...
    {sections.map((section, index) => (
      <div
        ...
        style={{
          flex: 1,
          minWidth: "280px",
          textAlign: section.type === "PDF" ? "center" : "left",
        }}
      >
        {section.type === "PDF" ? (
          <iframe
